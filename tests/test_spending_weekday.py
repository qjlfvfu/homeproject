...
class TestSpendingByWeekday:
    ...
    def sample_transactions(self):
        ...
        for date in dates:
            ...
            data.append({"date": date.strftime("%Y-%m-%d"), "amount": max(amount, 0)})
    ...
    def test_basic_functionality(self, sample_transactions):
        ...
        assert list(result["day_of_week"].values) == [
            0,
            1,
            2,
            3,
...
if __name__ == "__main__":
    ...
    for _, row in result_with_date.iterrows():
        print(
            f"{row['weekday_name']:12}: {row['avg_spending']:8.2f} руб. (медиана: {row['median_spending']:6.2f})"
        )
