...
class TestFinderFunction(unittest.TestCase):
    ...
    def test_finder_successful_search(self, mock_file):
        ...
        expected = {
            ...
            "found": 1,
            "results": [
                {"description": "Перевод денег", "category": "перевод", "amount": 100}
            ],
        }
...
class TestFinderPerformance(unittest.TestCase):
    ...
    def test_finder_large_dataset(self, mock_file):
        ...
        for i in range(1000):
            if i % 2 == 0:
                large_data.append(
                    {"description": f"Перевод {i}", "category": "перевод", "amount": i}
                )
            else:
                large_data.append(
                    {"description": f"Оплата {i}", "category": "оплата", "amount": i}
                )
...
class TestLoggerSetup(unittest.TestCase):
    ...
    def test_logger_creation(self, logging=None):
...
class TestFinderErrorLogging(unittest.TestCase):
    ...
    def test_finder_logs_json_decode_error(self, mock_logger):
        ...
        with patch(
            "builtins.open", new_callable=lambda: mock_open(read_data="invalid json")
        ):
