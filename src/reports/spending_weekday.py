...
def spending_by_weekday(
    ...
    try:
        ...
        logger.info(
            f"Анализ трат по дням недели за период с {start_date} по {end_date}"
        )
        ...
        filtered_transactions["weekday_name"] = filtered_transactions[
            "day_of_week"
        ].map(days_map)

        # Группируем по дню недели и считаем статистику
        result = (
            filtered_transactions.groupby(["day_of_week", "weekday_name"])
            .agg({"amount": ["mean", "sum", "count", "std"], "date": "nunique"})
            .round(2)
        )
        ...
        median_by_day = (
            filtered_transactions.groupby("day_of_week")["amount"].median().round(2)
        )
...
def create_empty_weekday_result(
